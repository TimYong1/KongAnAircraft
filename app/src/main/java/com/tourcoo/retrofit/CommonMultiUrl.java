...
import static com.tourcoo.retrofit.RetrofitConstant.EXCEPTION_EMPTY_URL;
...
public class CommonMultiUrl {
    ...
    private static volatile CommonMultiUrl sInstance;
    /**
     * retrofit 设置的BaseUrl
     */
    private String mBaseUrl;

    public static CommonMultiUrl getInstance() {
        if (sInstance == null) {
            synchronized (CommonMultiUrl.class) {
                if (sInstance == null) {
                    sInstance = new CommonMultiUrl();
                }
            }
        }
        return sInstance;
        ...
        this.mInterceptor = new Interceptor() {
            ...
            public Response intercept(Chain chain) throws IOException {
                ...
                if (request != null && request.url() != null && !request.url().toString().contains(mBaseUrl)) {
                    LogUtils.i(TAG, "无统一BaseUrl不拦截:" + request.url() + ";BaseUrl:" + mBaseUrl);
                    return chain.proceed(request);
                }
     ...
     * @return
     */
    public CommonMultiUrl with(OkHttpClient.Builder builder) {
    ...
    public Request processRequest(Request request) {
        ...
        if (null != httpUrl) {
            ...
            LogUtils.i(CommonMultiUrl.TAG, "Header 模式重定向Url:Base Url is { " + mBaseUrl + " }" + ";New Url is { " + newUrl + " }" + ";Old Url is { " + request.url() + " }");
            return newBuilder
                    .url(newUrl)
                    .build();
        }
        ...
        if (null != httpUrl) {
            LogUtils.i(CommonMultiUrl.TAG, "Method 模式重定向Url:Base Url is { " + mBaseUrl + " }" + ";New Url is { " + httpUrl + " }" + ";Old Url is { " + request.url() + " }");
            return newBuilder
                    .url(httpUrl)
                    .build();
        }
        ...
        if (httpUrlBase != null && !httpUrlBase.toString().equals(mBaseUrl)) {
            ...
            LogUtils.i(CommonMultiUrl.TAG, "重定向Url:Base Url is { " + httpUrlBase.toString() + " }" + ";New Url is { " + httpNew + " }" + ";Old Url is { " + request.url() + " }");
            return newBuilder
                    .url(httpNew)
                    .build();
        }
    ...
    private HttpUrl getMethodHttpUrl(Request request) {
        ...
        LogUtils.d(TAG, "Base Url is { " + mBaseUrl + " }" + ";Old Url is{" + url.newBuilder().toString() + "};Method is <<" + methodKey + ">>");
        //如果
     ...
     * @param enable
     */
    public CommonMultiUrl setIntercept(boolean enable) {
     ...
     * @return
     */
    public CommonMultiUrl setHeaderPriorityEnable(boolean enable) {
     ...
     * @param url
     */
    public CommonMultiUrl setGlobalBaseUrl(String url) {
    ...
    public CommonMultiUrl putHeaderBaseUrl(Map<String, String> map) {
     ...
     * @param urlValue
     */
    public CommonMultiUrl putHeaderBaseUrl(String urlKey, String urlValue) {
     ...
     * @param urlKey
     */
    public CommonMultiUrl removeHeaderBaseUrl(String urlKey) {
        synchronized (mHeaderBaseUrlMap) {
     ...
     * 清除所有BaseUrl
     */
    public CommonMultiUrl clearAllHeaderBaseUrl() {
     ...
     * @param parser
     */
    public CommonMultiUrl setUrlParser(UrlParser parser) {
        this.mUrlParser = parser;
        return sInstance;
    }

    public CommonMultiUrl putBaseUrl(Map<String, String> map) {
        if (map != null && map.size() > 0) {
            for (Map.Entry<String, String> entry : map.entrySet()) {
     ...
     * @return
     */
    public CommonMultiUrl putBaseUrl(String urlKey, String urlValue) {
        synchronized (mBaseUrlMap) {
    ...
    public CommonMultiUrl removeBaseUrl(String urlKey) {
        synchronized (mBaseUrlMap) {
     ...
     * 清除所有BaseUrl
     */
    public CommonMultiUrl clearAllBaseUrl() {
        mBaseUrlMap.clear();
        return sInstance;
    }
    ...
    private String getHeaderBaseUrlKey(Request request) {
        ...
        if (heads != null) {
            LogUtils.i(TAG, "header:" + heads.toString());
        }
    ...
    private HttpUrl checkUrl(String url) {
        ...
        if (null == parseUrl) {
            throw new NullPointerException(EXCEPTION_EMPTY_URL);
        } else {
            return parseUrl;
        }
    }
